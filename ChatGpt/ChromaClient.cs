...
public class ChromaDirect
{
    ...
    public async Task UpsertAsync(string collectionId, List<(string Id, string Text)> documents)
    {
        ...
        // 1) Фильтрация: убираем пустые Id/тексты
    ...
    public async Task<List<float[]>> GetEmbeddingsAsync(IEnumerable<string> texts)
    {
        try
        {
            ...
            foreach (var raw in texts ?? Enumerable.Empty<string>())
            {
                ...
                // Запрещаем совсем пустые строки
                if (string.IsNullOrEmpty(text))
                    throw new ArgumentException("Embedding input contains an empty string.");

                var payload = new { model = "nomic-embed-text:latest", input = text };
                var content = new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json");
                ...
                {
                    var embedding = emb.EnumerateArray().Select(e => e.GetSingle()).ToArray();
    ...
    string mustContain2 = null)
    {
        try
        {
            ...
            // where_document: $contains по шапке чанка
            object whereDocument = null;
            var clauses = new List<Dictionary<string, object>>();
            if (!string.IsNullOrWhiteSpace(mustContain1)) clauses.Add(new Dictionary<string, object> { ["$contains"] = mustContain1 });
            if (!string.IsNullOrWhiteSpace(mustContain2)) clauses.Add(new Dictionary<string, object> { ["$contains"] = mustContain2 });
            ...
            // Лёгкий реранк + фолбэк
            var scored = docs.Select((txt, i) =>
            {
                double baseScore = (i < dists.Count) ? -dists[i] : 0; // меньше distance = лучше
                double bonus = 0;
            ...
            // если после бонусов ничего не попало — вернём просто топ-N
